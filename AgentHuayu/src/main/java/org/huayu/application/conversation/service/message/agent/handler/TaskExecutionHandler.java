package org.huayu.application.conversation.service.message.agent.handler;
...
import org.huayu.application.conversation.service.message.Agent;
import org.huayu.application.conversation.service.message.agent.event.AgentWorkflowEvent;
import org.huayu.application.conversation.service.message.agent.manager.AgentToolManager;
import org.huayu.application.conversation.service.message.agent.manager.TaskManager;
import org.huayu.application.conversation.service.message.agent.template.AgentPromptTemplates;
...
import org.huayu.domain.task.constant.TaskStatus;
...
public class TaskExecutionHandler extends AbstractAgentHandler {
    ...
    private <T> void executeSubTask(
        ...
        try {
            ...
            // 构建任务提示词
            String taskPrompt = AgentPromptTemplates.getTaskExecutionPrompt(
                    userRequest,
                    taskName,
                    previousTaskResults);
            
            // 执行子任务
            ChatModel model = llmServiceFactory.getStrandClient(
    ...
    private <T> void handleToolCalls(AiMessage aiMessage, AgentWorkflowContext<T> context) {
        // 创建工具调用消息实体
        MessageEntity toolCallMessageEntity = createMessageEntity(
                context, MessageType.TOOL_CALL, null, 0);
