...
public class ChatMessageHandler extends AbstractMessageHandler {
    ...
    @Override
    public <T> T chat(ChatContext environment, MessageTransport<T> transport) {
        // 创建用户消息实体
     ...
     * 创建LLM消息实体
     */
    private MessageEntity createLlmMessage(ChatContext environment) {
        MessageEntity messageEntity = new MessageEntity();
     ...
     * 准备LLM请求
     */
    private dev.langchain4j.model.chat.request.ChatRequest prepareLlmRequest(ChatContext environment) {
        // 构建聊天消息列表
